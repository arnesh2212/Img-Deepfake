...
class FreqDINO_ablation0(nn.Module):
    # Ablation 0: FreqDINO with Cross-Modal Contrastive Loss
    # Same architecture as baseline but adds contrastive loss between noise and DINO features
    def __init__(self, n_fusion_layers=3, n_heads=8, d_model=768,
                 dino_embed_size=1024, n_patches=196, clip_layers=12, num_classes=3,
                 n_bands=3):
        super().__init__()
    ...
    def forward(self, image, clip_embed, dino_cls, dino_reg, dino_patch, has_mask=None, mask=None):
        ...
        noise_pooled = noise_proj_dmodel.mean(dim=1)
        noise_contrast = self.noise_proj_contrast(noise_pooled)
        noise_contrast = F.normalize(noise_contrast, p=2, dim=1)
        
        dino_contrast = self.dino_proj_contrast(fused_dino_cls)
        dino_contrast = F.normalize(dino_contrast, p=2, dim=1)
...
if __name__ == "__main__":
    ...
    moel_ab0 = FreqDINO_ablation0()
    out_ab0 = moel_ab0(image, clip_embed, dino_cls, dino_reg, dino_patch, 
                has_mask=has_mask, mask=mask)
    print("Ab0 - Logits shape:", out_ab0["logits"].shape)
    print("Ab0 - Seg logits shape:", out_ab0["seg_logits"].shape)
